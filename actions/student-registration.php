...
if(isset($_POST['type']) && $_POST['type'] == 'student' && isset($_POST['email']) && !empty($_POST['email']))
{
    ...
    $password = date('dmY', strtotime($dob)); // Using the date of birth as the password
    ...
    {    
        $password = $father_mobile; // Using the father's mobile number directly as the password
        $query = mysqli_query($db_conn, "INSERT INTO accounts (`name`,`email`,`password`,`type`) VALUES ('$father_name','$father_mobile','$password','parent')") or die(mysqli_error($db_conn));
        if($query)
        {
